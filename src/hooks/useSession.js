...
const useSession = () => {
  ...
  // -----------------------------------------------------------------------
  // ⚡ YENİ: Seviye sinyalini diske kaydet (her seviye bitince çağrılır)
  // -----------------------------------------------------------------------
  /**
   * Bir seviye (kolay/orta/zor) bitince, o seviyenin EEG sinyalini
   * backend'de session_data klasörüne kaydeder (eeg_kolay.txt gibi).
   * Fire-and-forget — deney akışını bloklamaz.
   */
  const saveLevelEEG = useCallback((level) => {
    if (!sessionIdRef.current) return;

    _post(`/api/session/${sessionIdRef.current}/save-level?level=${encodeURIComponent(level)}`, {})
      .then(res => res.json())
      .then(data => {
        console.log(`[useSession] ✓ Seviye sinyali kaydedildi (${level}):`,
          data.sample_count, 'sample →', data.files_created);
      })
      .catch(err => {
        console.warn(`[useSession] Seviye sinyali kaydedilemedi (${level}):`, err.message);
      });
  }, [_post]);

  // -----------------------------------------------------------------------
  // ⚡ YENİ: Tüm session'ı diske kaydet (sınav sonunda)
  // -----------------------------------------------------------------------
  const finalizeSession = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      console.log('[useSession] ✓ Session finalized:', data.folder_path, data.files_created);
      return data;
    } catch (err) {
      console.warn('[useSession] Finalize başarısız:', err.message);
      return null;
    }
  }, [_post]);
  ...
  return {
    ...
    // Session
    createSession,
    finalizeSession,      // ⚡ Yeni — sınav sonu otomatik kayıt
    reset,
    ...
    saveLevelEEG,         // ⚡ Yeni — her seviye bitince sinyal kaydı
